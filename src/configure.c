...
static CfgResult help_protocols(CfgResult exit_code)
{
    ...
    for (int i = 0; i < num_r_devices; i++) {
        ...
        if (!getDev(i, &devptr)) {
            fprintf(stderr, "    [%02d] - error, could not access device information.\n", i + 1);
        }
        else {
...
static CfgResult parse_conf_option(r_cfg_t *cfg, int opt, char *arg) {
    ...
    case 'b':{
        ...
        if (val % 512) {
            val -= (val % 512);
            if (!val) val = 512;
            fprintf(stderr, "-b param must be multiple of 512. Falling back to %d\n", val);
        }
    ...
    case 'F': {
        ...
        if (strncmp(arg, "json", 4) == 0) {
            strcpy(cfg->output_path_json, (out ? out : "")); // empty string forces output to stdout
            cfg->outputs_configured |= OUTPUT_JSON;
        }
        else if (strncmp(arg, "csv", 3) == 0) {
            strcpy(cfg->output_path_csv, (out ? out : "")); // empty string forces output to stdout
            cfg->outputs_configured |= OUTPUT_CSV;
        }
        else if (strncmp(arg, "kv", 2) == 0) {
            strcpy(cfg->output_path_kv, (out ? out : "")); // empty string forces output to stdout
            cfg->outputs_configured |= OUTPUT_KV;
        }
            ...
            if (opts) { // might be ""
                ...
                fprintf(stderr, "Bad host/port param.\n");
                return CFG_EXITCODE_ONE;
            }
        }
            ...
            if (hostport_param(arg_param(arg), &host, &port)) {
                ...
                strcpy(cfg->output_udp_host, host);
                strcpy(cfg->output_udp_port, port);
